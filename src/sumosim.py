...
class SumoSim:
    def __init__(self, cfg_fp, sim_len, tsc, nogui, netdata, args, idx):
        print('[mylog][sumosim.py][20] SumoSim:__init__(self, ...')
        self.cfg_fp = cfg_fp
    ...
    def gen_sim(self):
        print('[mylog][sumosim.py][29] def gen_sim(self) ... ')
        ...
        port = self.args.port+self.idx
        print('[mylog][sumosim.py][38] self.sumo_process = subprocess.Popen([sumoBinary, "-c", ...')
        self.sumo_process = subprocess.Popen([sumoBinary, "-c",
                                         self.cfg_fp, "--remote-port",
                                         str(port), "--no-warnings",
                                         "--no-step-log", "--random"],
                                         stdout=None, stderr=None)
        print('[mylog][sumosim.py][44] self.conn = traci.connect(port) ...')
