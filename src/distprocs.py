...
class DistProcs:
    def __init__(self, args, tsc, mode):
        print('[mylog] [distprocs.py][27] DistProcs __init__(self, arg ...')
        self.args = args
        rl_tsc = ['ddpg', 'dqn']
        traditional_tsc = ['websters', 'maxpressure', 'sunpressure','normpressure', 'sotl', 'uniform']
        ...
        print('[mylog] [distprocs.py][70] sim = SumoSim(args.cfg ...')
        sim = SumoSim(args.cfg_fp, args.sim_len, args.tsc, True, netdata, args, -1)
        print('[mylog] [distprocs.py][72] sim.gen_sim() ... (dummy)')
        sim.gen_sim()
        ...
        print('[mylog][distprocs.py][91] sim_procs = [ SimProc ( ... (machi_dummy)')
        ...
        #learner_procs = []
        print('[mylog][distprocs.py][106] self.procs = sim_procs + learner_procs ...')
    ...
    def run(self):
        ...
        ###start everything   
        for p in self.procs:
            print('[mylog][distprocs.py][114] p.start() ...')
            p.start()
